...
async function bootstrap() {
  ...
  const logger = crearLogger("ventas-caja-service");
  const puerto = process.env.PORT ?? 4005;

  app.useGlobalPipes(new ValidationPipe({ whitelist: true, transform: true }));
  app.useGlobalFilters(new FiltroExcepcionesHttp());

  const config = new DocumentBuilder()
    .setTitle("SCIPOS - Servicio de Ventas POS y Caja")
    .setDescription(
      "Ventas del punto de venta y operación de caja. Las peticiones protegidas " +
        "identifican al usuario con el header x-usuario-id.",
    )
    .setVersion("1.0")
    .build();
  ...
  logger.log(`Servicio de ventas-caja escuchando en http://localhost:${puerto}`);
