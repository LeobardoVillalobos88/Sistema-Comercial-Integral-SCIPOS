...
async function bootstrap() {
  ...
  const logger = crearLogger("productos-service");
  ...
  const config = new DocumentBuilder()
    .setTitle("SCIPOS - Servicio de Productos")
    .setDescription(
      "Catálogo de productos y servicios, inventario y compras a proveedor. " +
  ...
  logger.log(`Servicio de productos escuchando en http://localhost:${puerto}`);
