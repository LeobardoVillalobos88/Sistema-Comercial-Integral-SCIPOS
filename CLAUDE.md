...
The **frontend (Avance 2) is built and runnable**, and the **backend (Avance 3) is partially built**: gateway, `seguridad`, `productos` (4002, includes compras/inventario) and `clientes` (4003, includes historial distribuido) are merged and working; `cotizaciones` (4004) and `ventas-caja` (4005) are **in progress** on their feature branches, per `docs/readmes/avance-3-plan-backend.md`. The `productos` and `clientes` microfrontends and the dashboard stat cards consume the real API; cotizaciones and POS/caja still run on mocks. Mobile is not started. `pnpm-workspace.yaml` includes `apps/frontend/*`, `apps/backend/*`, `apps/backend/services/*`, and `packages/*`.
...
apps/
  ...
  backend/          # BASE EXISTS
    ...
    services/       # EXISTS: seguridad (4001), productos (4002), clientes (4003), example-service (template)
                    # PENDING: cotizaciones 4004, ventas-caja 4005 (in progress on feature branches)
