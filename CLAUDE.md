...
- `PermisosProvider.tsx` — downloads users from `GET /seguridad/usuarios` and the active user's effective privileges from `GET /seguridad/usuarios/:id/privilegios`. The login screen signs in a **seed user** (with per-role quick-access buttons); `can()` checks the downloaded list. If the API is unreachable it falls back to the local `matriz.ts` so the UI stays navigable (`origenPermisos: "api" | "local"` tells you which).
