...
- **`productos-front/`** (`@scipos/productos-front`, port 3003) — RF-07/08/09: `CatalogoProductos` with search, estado/tipo filters, create/edit dialog (lote, caducidad, **precioCompra + precioVenta**, real-time input validation), soft-delete toggle, and hard delete (Admin-only `productos:eliminar`, confirm + toast). Fully backed by the productos API via `llamarApi` (waits for permisos before fetching; `SkeletonTabla` while loading).
...
- **`services/cotizaciones/`** (`@scipos/cotizaciones-service`, port 4004, schema `cotizaciones`) — quotes with a transactional sequential folio (`SecuenciaFolio` upsert, `COT-000001` style), **BORRADOR → ENVIADA → VENDIDA** lifecycle, and totals computed server-side with `decimal.js` (`calculos-cotizacion.ts`, unit-tested — the repo's first tests, run with `pnpm --filter @scipos/cotizaciones-service test`, plain `node:test` via `tsx`, no extra test runner dependency). `POST /:id/convertir` calls ventas-caja's `POST /ventas/convertir-cotizacion` inside a Postgres advisory lock (`pg_advisory_xact_lock`) and only flips the quote to VENDIDA if the sale was actually created; retrying an already-converted quote returns the same result instead of erroring. Validates client and every product exist (and are active) via `ClientesClient`/`ProductosClient` before creating anything. `GET /cotizaciones/resumen` for the dashboard.
...
- **`services/example-service/`** (`@scipos/example-service`, template) — copy to create a domain service; its README documents the steps (rename, port, own Postgres schema via `DATABASE_URL` `?schema=`, replace the `Ejemplo` resource, seed with frontend-mock IDs, register in the gateway routing table, write the OpenAPI contract first). Every service defines a `preparar` script (generate + migrate deploy + seed) that `pnpm setup:backend` runs.
...
- **JWT auth is live**: `POST /auth/login` (seguridad) validates bcrypt credentials and signs a JWT (`JWT_SECRETO`, same value in every service's `.env`; `sub` = user id). `ExtractorIdentidadJwt` (the default in `ModuloSeguridad.registrar()`) verifies `Authorization: Bearer` and falls back to `x-usuario-id` **only as the inter-service channel** — the gateway strips that header from all external requests, so outside identity must be a signed token. `GET /auth/perfil` restores a session from a stored token. Controllers never read headers directly; they use the `@UsuarioActual()` param decorator (guard-resolved id). The login screen is pending (a teammate owns it): it only needs to call `iniciarSesion(correo, contrasena)` from `usePermisos()` — the topbar role selector currently logs in with the seed credentials (`CREDENCIALES_DEMO` in commons) so all traffic is token-signed already. Session persists per-tab in `sessionStorage`.
...
pnpm --filter @scipos/cotizaciones-service test # unit tests (node:test via tsx)
...
- **Only `cotizaciones-service` has tests** so far (`node:test` run via `tsx`, no extra dependency — see `calculos-cotizacion.spec.ts`/`cotizaciones.service.spec.ts`). `pnpm test` (turbo `test` task) runs whatever `test` script each package defines; packages without one are a no-op. Follow this same pattern (`tsx --test src/**/*.spec.ts`, no new devDependency) when adding tests elsewhere.
...
- Shared UI components (`PageHeader`, `SearchableTable`, `SkeletonTabla`, `EstadoChip`, `EstadoCotizacionChip`, `StatCard`, `Permiso`), feedback (`useToast`, `confirmar` — from `/feedback`), and helpers (`formatearMoneda`, `formatearFecha`, `formatearFechaConHora`) all live in and are re-exported from `commons`. `Producto` carries `lote`, `fechaCaducidad?`, `precioCompra`, `precioVenta` (there is no single `precio` field). `PRODUCTOS_MOCK`/`CLIENTES_MOCK` remain in `commons/mocks` only as the `example-front` template's sample data and as an offline fallback for `/inicio` and the dashboard stat cards when their service is down — every real domain module (`productos-front`, `clientes-front`, `cotizaciones-front`, `pos-caja-front`) is fully API-backed. `COTIZACIONES_MOCK` and the `totalCotizacion()` helper were removed once cotizaciones-front connected to the real API (the backend computes totals now); don't recreate them.
...
| Auth | JWT, roles **and** dynamic privileges — **in place** (login + bcrypt + Bearer verification in every service; JWKS remains an optional advanced tier) |
...
apps/
  ...
  backend/          # BASE EXISTS
    ...
    services/       # EXISTS: seguridad (4001), productos (4002), clientes (4003), cotizaciones (4004), ventas-caja (4005), reportes (4006, sin BD), example-service (template)
...
**Every scope in the brief is done and verified end-to-end**: JWT auth + dynamic privileges, productos, clientes, cotizaciones + conversión a venta (idempotent, stock-decrementing), POS/caja, compras, comprobante PDF and reportes/utilidad (the dashboard's "Utilidad de hoy" card now reads the real `/reportes/utilidad` for today). Remaining ideas are optional advanced tiers only (Kafka events, JWKS, CI/CD). Don't let the privilege system degrade — it's the graded centerpiece.
