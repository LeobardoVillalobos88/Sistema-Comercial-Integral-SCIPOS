...
pnpm --filter @scipos/web-shell dev           # just the host        → http://localhost:3001pnpm --filter @scipos/productos-front dev     # just productos       → http://localhost:3003
...
- **`cotizaciones-service` and `seguridad-service` have tests** (`node:test` run via `tsx`, no extra dependency — see `calculos-cotizacion.spec.ts`/`cotizaciones.service.spec.ts` and `privilegios.service.spec.ts`). The seguridad suite pins the graded rule: effective privileges are the role's plus the granted minus the revoked, and an individual revocation beats even a role with `accesoTotal`. It injects plain objects cast as `PrismaService`/`RedisService`, so it needs no database or Redis. `pnpm test` (turbo `test` task) runs whatever `test` script each package defines; packages without one are a no-op. Follow this same pattern (`tsx --test src/<modulo>/*.spec.ts`, no new devDependency) when adding tests elsewhere.
