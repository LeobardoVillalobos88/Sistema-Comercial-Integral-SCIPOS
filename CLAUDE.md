...
The **frontend (Avance 2) is built and runnable**, and the **backend (Avance 3) is complete for the protected core scope**: gateway plus all five domain services — `seguridad` (4001), `productos` (4002, includes compras/inventario), `clientes` (4003, includes historial distribuido), `cotizaciones` (4004, includes conversión a venta) and `ventas-caja` (4005, POS + caja) — are merged and working, per `docs/readmes/avance-3-plan-backend.md`. Every microfrontend (`productos-front`, `clientes-front`, `cotizaciones-front`, `pos-caja-front`) and the dashboard stat cards consume the real API; no domain module runs on mocks anymore (`example-front` keeps mock data deliberately, as the template). Mobile is not started. `pnpm-workspace.yaml` includes `apps/frontend/*`, `apps/backend/*`, `apps/backend/services/*`, and `packages/*`.
...
| Backend | Microservices — NestJS + Prisma — **in place**: gateway + 5 domain services (seguridad, productos, clientes, cotizaciones, ventas-caja) + commons + template |
...
apps/
  ...
  backend/          # BASE EXISTS
    ...
    services/       # EXISTS: seguridad (4001), productos (4002), clientes (4003), cotizaciones (4004), ventas-caja (4005), example-service (template)
...
The protected core — **auth + dynamic privileges → productos → clientes → cotizaciones → conversión cotización-a-venta** — is **done and verified end-to-end** (create quote → send → convert → real sale created → stock decremented → quote marked VENDIDA → distributed client history shows it, with retries idempotent). POS/caja is also connected. What's left: comprobante PDF and reportes/dashboard-de-utilidad (no service exists for either yet — the dashboard's "Utilidad de hoy" card is still a hardcoded placeholder for this reason). Don't let the privilege system degrade — it's the graded centerpiece.
