...
- **`productos-front/`** (`@scipos/productos-front`, port 3002) — the first domain microfrontend, fully scaffolded and integrated. Implements RF-07/RF-08/RF-09: `CatalogoProductos` component with search, estado/tipo filters, create/edit dialog, and soft-delete toggle — all guarded by `productos:*` privileges. Data is in-memory (`PRODUCTOS_MOCK`); no API calls yet. Shares port 3002 with `example-front` — the two are not meant to run standalone at the same time.
...
pnpm --filter @scipos/productos-front dev     # just productos       → http://localhost:3002
